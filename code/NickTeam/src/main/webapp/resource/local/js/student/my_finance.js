		...
		for (var i = 0; i < list.length; i++) {
			...
			if ("recharge" == object.type) {
				if ("waitJudgement" == object.status
						|| "passJudgement" == object.status) {
					html += "<td style='width:12%;'><a href='" + G_basePath
							+ "/router/toRecharge2.action?detailId="
							+ object.id + "'>" + object.typeStr + "</a></td>";
				...
				if ("waitJudgement" == object.status
						|| "passJudgement" == object.status) {
					html += "<td style='width:12%;'>" + object.typeStr
							+ "</td>";
					html += "<td class='b-red' style='width:15%;'><s class='s-cat-41'></s>"
							+ object.price + "</td>";
				} else {
			...
			if ("waitJudgement" == object.status
					|| "passJudgement" == object.status) {
				html += "<td class='' style='width:13%;'>"
						+ object.statusStr + "</td>";
			} else {
				html += "<td class='b-red' style='width:13%;'>"
						+ object.statusStr + "</td>";
			}
			if ("waitJudgement" == object.status) {
...
function toWithdraw() {
	...
	if (accountId) {
		window.location.href = G_basePath
				+ "/router/toWithdraw.action?accountId=" + accountId;
	}
}