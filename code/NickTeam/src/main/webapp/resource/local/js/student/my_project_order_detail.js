...
function putProjectOrderProcessLogToPage(list, elementId) {
	...
	if (list && 0 < list.length) {
		for (var i = 0; i < list.length; i++) {
			...
			} else if ("interViewFailed" == object.nextStatus && 0 == i) {
				html += "<td><a href='javascript:showInterViewResult();'>查看面试结果</a></td>";
			} else {
				if ("waitProjectPay" == object.nextStatus) {
					html += "<td><a href='javascript:showInterViewResult();'>查看面试结果</a></td>";
				} else {
...
function showInterViewResult() {
	if (page_oderInfo) {
		$('#interview_result_dialog').show();
		$('#interview_result_dialog').window({
			width : 900,
			height : 150,
		});
		console.log(page_oderInfo.interviewResult);
		if (page_oderInfo.interviewResult && page_oderInfo.interviewFlagStr) {
...
function showInterViewInfo() {
	if (page_oderInfo) {
		...
		if (page_oderInfo.interviewer) {
			$("#v_interviewer_username").html(
					page_oderInfo.interviewer.username);
		}
	} else {
...
function showCosts(isNeedAirTicket, airTicketInfo, costsElementId,
	...
	if ("stepable" == page_oderInfo.payType) { // 按流程支付报名费
		for (var i = 0; i < page_project_costs.length; i++) {
			if ("entryfee" != page_project_costs[i].type) {
				costHtml += page_project_costs[i].typeStr + ":"
